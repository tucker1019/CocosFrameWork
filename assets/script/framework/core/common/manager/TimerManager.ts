...
export class TimerManager extends EventDispatcher {
     ...
     * @param uuid  唯一标识
     * @example
    var uuid = frame.timer.schedule(()=>{
        // 每秒触发一次
    }, 1000);

    // 删除指定标识的触发器
    frame.timer.unschedule(uuid);
     */
    ...
    export class Test extends Component {
        ...
        start() {
            // 在指定对象上注册一个倒计时的回调管理器
            this.timeId = frame.timer.register(this, "countDown", this.onSecond, this.onComplete);
        }
    ...
    export class Test extends Component {
        ...
        onDestroy() {
            // 在指定对象上注销一个倒计时的回调管理器
            frame.timer.unRegister(this.timeId);
        }
    }
     */
